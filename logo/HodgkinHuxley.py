...
class HodgkinHuxley(Model):
    def __init__(self):
        Model.__init__(self, interpolate=False, labels=["Time (ms)", "Voltage (mV)"])
    ...
    # K channel
    def alpha_n(self, V):
        return 0.01 * (V + 55) / (1 - np.exp(-(V + 55) / 10.0))
    ...
    def n_inf(self, V):
        return self.alpha_n(self.V_rest) / (
    ...
    def m_inf(self, V):
        return self.alpha_m(self.V_rest) / (
            self.alpha_m(self.V_rest) + self.beta_m(self.V_rest)
        )

    # Na channel (inactivating)
    def alpha_h(self, V):
        return 0.07 * np.exp(-(V + 65) / 20.0)

    def beta_h(self, V):
        return 1 / (np.exp(-(V + 35) / 10.0) + 1)

    def h_f(self, h, V):
        return self.alpha_h(V) * (1 - h) - self.beta_h(V) * h

    def h_inf(self, V):
        return self.alpha_h(self.V_rest) / (
