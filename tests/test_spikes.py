...
class TestSpikes(TestCasePlot):
    ...
    def test_find_spikes_default_auto_extended(self):
        ...
        self.spikes.find_spikes(
            self.time, self.values, threshold="auto", extended_spikes=True
        )
        self.assertEqual(self.spikes.nr_spikes, 12)
    ...
    def test_find_spikes_normalize(self):
        ...
        self.spikes.find_spikes(
            self.time, self.values, threshold=0.4, end_threshold=-0.1, normalize=True
        )
        self.assertEqual(self.spikes.nr_spikes, 12)
    ...
    def test_plot_spikes(self):
        self.spikes = Spikes(
            self.time, self.values, xlabel="xlabel", ylabel="ylabel"
        )
