...
class TestParallel(unittest.TestCase):
    def setUp(self):
        ...
        self.parallel = Parallel(
            model=TestingModel1d(), features=self.features, logger_level="error"
        )
    ...
    def test_feature_function(self):
        ...
        self.assertEqual(
            self.parallel.features.features_to_run, ["feature_function"]
        )
    ...
    def test_feature_functions(self):
        ...
        self.assertEqual(
            self.parallel.features.features_to_run,
    ...
    def test_create_interpolations(self):
        results = {
            ...
            "feature1d": {"values": np.arange(0, 10), "time": np.arange(0, 10)},
            ...
            "feature_invalid": {"values": np.nan, "time": np.nan},
        }

        results = self.parallel.create_interpolations(results)

        self.assertTrue(
            np.array_equal(results["TestingModel1d"]["time"], np.arange(0, 10))
        )
        self.assertTrue(
            np.array_equal(results["TestingModel1d"]["values"], np.arange(0, 10) + 1)
        )
        self.assertTrue(
        ...
        self.assertTrue(
            np.array_equal(results["feature_interpolate"]["time"], np.arange(0, 10))
        )
        self.assertTrue(
            np.array_equal(
                results["feature_interpolate"]["values"], np.arange(0, 10) + 1
            )
        )
        self.assertIsInstance(
            results["feature_interpolate"]["interpolation"],
    ...
    def test_create_interpolations_irregular_values(self):
        ...
        results = {
            "TestingModel1d": {"values": [1, 3, [3, 4]], "time": np.arange(0, 10)}
        }
    ...
    def test_create_interpolations_irregular_time(self):
        ...
        results = {
            "TestingModel1d": {"values": np.arange(0, 10), "time": [1, 3, [3, 4]]}
        }
    ...
    def test_create_interpolations_ignore(self):
        ...
        self.assertIsInstance(
            inter["TestingModel1d"]["interpolation"],
            scipy.interpolate.fitpack2.UnivariateSpline,
        )
    ...
    def test_create_interpolations_feature_1d_no_t(self):
        results = {
            "feature_interpolate": {"values": np.arange(0, 10), "time": np.nan}
        }
    ...
    def test_create_interpolations_feature_0d(self):
        results = {"feature_interpolate": {"values": 1, "time": np.arange(0, 10)}}
    ...
    def test_interpolation_1d(self):
        ...
        self.assertIsInstance(
            interpolation, scipy.interpolate.fitpack2.UnivariateSpline
        )

    def test_interpolation_1d_different_time_values(self):
        results = {
            "TestingModel1d": {"values": np.arange(0, 5), "time": [[1, 3], [3, 4]]}
        }
    ...
    def test_interpolation_1d_values_none(self):
        results = {
            "TestingModel1d": {"values": [1, 2, 3, 4, None], "time": [1, 2, 3, 4, 5]}
        }
    ...
    def test_interpolation_1d_time_none(self):
        results = {
            "TestingModel1d": {"time": [1, 2, 3, 4, None], "values": [1, 2, 3, 4, 5]}
        }
    ...
    def test_interpolation_1d_feature_1d_no_t(self):
        results = {"TestingModel1d": {"values": np.arange(0, 10), "time": np.nan}}
    ...
    def test_interpolation_1d_feature_1d_t_nan(self):
        results = {
            "TestingModel1d": {"values": np.arange(0, 10), "time": [np.nan] * 10}
        }
    ...
    def test_run_interpolate(self):
        ...
        self.assertTrue(
            np.array_equal(results["TestingModelAdaptive"]["time"], np.arange(0, 11))
        )
        self.assertTrue(
            np.array_equal(
                results["TestingModelAdaptive"]["values"], np.arange(0, 11) + 1
            )
        )
        self.assertIsInstance(
            results["TestingModelAdaptive"]["interpolation"],
            scipy.interpolate.fitpack2.UnivariateSpline,
        )

        self.assertTrue(
            np.array_equal(results["feature_interpolate"]["time"], np.arange(0, 11))
        )
        self.assertTrue(
            np.array_equal(
                results["feature_interpolate"]["values"], np.arange(0, 11) + 1
            )
        )
        self.assertIsInstance(
            results["feature_interpolate"]["interpolation"],
