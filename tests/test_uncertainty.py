...
class TestUncertainty(TestCasePlot):
    ...
    def test_init(self):
        uncertainty = UncertaintyQuantification(
            self.model, self.parameters, logger_level="error", logger_filename=None
        )
    ...
    def test_init_parameter_error(self):
        with self.assertRaises(TypeError):
            UncertaintyQuantification(
                self.model, 2, logger_level="error", logger_filename=None
            )

    def test_init_backend_error(self):
        with self.assertRaises(ValueError):
            UncertaintyQuantification(
                self.model, self.parameter_list, backend="not a backend"
            )
    ...
    def test_init_uncertainty_calculations_custom(self):
        ...
        result = (
            uncertainty.uncertainty_calculations.custom_uncertainty_quantification()
        )
    ...
    def test_init_uncertainty_calculations(self):
        ...
        self.assertIsInstance(
            uncertainty.uncertainty_calculations, TempUncertaintyCalculations
        )
    ...
    def test_set_parameters(self):
        ...
        self.assertIsInstance(uncertainty.parameters, Parameters)
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.parameters, Parameters
        )
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.runmodel.parameters, Parameters
        )
    ...
    def test_set_features(self):
        ...
        self.assertIsInstance(uncertainty.features, TestingFeatures)
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.features, TestingFeatures
        )
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.runmodel.features, TestingFeatures
        )
    ...
    def test_feature_function(self):
        ...
        self.assertEqual(
            self.uncertainty.features.features_to_run, ["feature_function"]
        )
    ...
    def test_feature_functions(self):
        ...
        self.assertEqual(
            self.uncertainty.features.features_to_run,
    ...
    def test_feature_functions_base(self):
        ...
        self.uncertainty.features = SpikingFeatures(
            [feature_function, feature_function2]
        )
        ...
        self.assertEqual(
            set(self.uncertainty.features.features_to_run),
    ...
    def test_set_model(self):
        ...
        self.assertIsInstance(uncertainty.model, TestingModel1d)
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.model, TestingModel1d
        )
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.runmodel.model, TestingModel1d
        )
    ...
    def test_set_model_function(self):
        ...
        self.assertIsInstance(
            uncertainty.uncertainty_calculations.runmodel.model, Model
        )
    ...
    def test_create_PCE_custom(self):
        def create_PCE_custom(self, uncertain_parameters=None):
            ...
            data.add_features(
                ["TestingModel1d", "feature0d_var", "feature1d_var", "feature2d_var"]
            )
        ...
        data = uncertainty.polynomial_chaos(
            method="custom",
            data_folder=self.output_test_dir,
            figure_folder=self.output_test_dir,
        )

        self.assertTrue(uncertainty.data.test_value, "custom PCE method")

        self.assertTrue(data.test_value, "custom PCE method")
        ...
        for data_type in data_types:
            if data_type not in ["evaluations", "time", "labels"]:
                for feature in uncertainty.data:
                    self.assertIsInstance(
                        uncertainty.data[feature][data_type], np.ndarray
                    )
    ...
    def test_convert_uncertain_parameters_list(self):
        result = (
            self.uncertainty.uncertainty_calculations.convert_uncertain_parameters(
                ["a", "b"]
            )
        )
    ...
    def test_polynomial_chaos_single(self):
        data_dict = self.uncertainty.polynomial_chaos_single(
            data_folder=self.output_test_dir,
            figure_folder=self.output_test_dir,
            seed=self.seed,
        )
        ...
        result = subprocess.call(
            ["h5diff", "-d", str(self.difference_treshold), filename, compare_file]
        )
    ...
    def test_PC_model_function(self):
        ...
        data = self.uncertainty.polynomial_chaos(
            data_folder=self.output_test_dir,
            figure_folder=self.output_test_dir,
            seed=self.seed,
        )
    ...
    def test_var_monte_carlo_single(self):
        ...
        data_dict = self.uncertainty.monte_carlo_single(
            filename="TestingModel1d_MC",
            plot=None,
            data_folder=self.output_test_dir,
            seed=self.seed,
    ...
    def test_monte_carlo(self):
        ...
        data = self.uncertainty.monte_carlo(
            filename="TestingModel1d_MC",
            plot=None,
            data_folder=self.output_test_dir,
            seed=self.seed,
    ...
    def test_load(self):
        ...
        for data_type in data_types:
            self.assertTrue(
                np.array_equal(
                    self.uncertainty.data["feature1d"][data_type], [1.0, 2.0]
                )
            )
            self.assertTrue(
                np.array_equal(
                    self.uncertainty.data["TestingModel1d"][data_type], [3.0, 4.0]
                )
            )
        ...
        self.assertTrue(
            np.array_equal(
                self.uncertainty.data["TestingModel1d"]["labels"],
                ["xlabel", "ylabel"],
            )
        )
        self.assertTrue(
            np.array_equal(
                self.uncertainty.data["feature1d"]["labels"], ["xlabel", "ylabel"]
            )
        )
    ...
    def test_plotNoSensitivity(self):
        ...
        self.uncertainty.plot(
            type="condensed_no_sensitivity", folder=self.output_test_dir
        )
    ...
    def test_set_uncertainty_calculations(self):
        ...
        self.assertIsInstance(
            self.uncertainty.uncertainty_calculations.model, TestingModel1d
        )
        self.assertIsInstance(
            self.uncertainty.uncertainty_calculations.features, TestingFeatures
        )

    def test_quantifyPCAll(self):
        self.set_up_test_calculations()

        data = self.uncertainty.quantify(
            method="pc",
            data_folder=self.output_test_dir,
            figure_folder=self.output_test_dir,
            seed=self.seed,
        ...
        self.assertEqual(
            self.uncertainty.data.arguments["uncertain_parameters"], ["a", "b"]
        )
        ...
        self.assertEqual(
            self.uncertainty.data.arguments["nr_pc_mc_samples"], 10**3
        )
    ...
    def test_no_save(self):
        ...
        self.uncertainty.monte_carlo_single(
            plot=None,
            save=False,
            data_folder=self.output_test_dir,
            figure_folder=self.output_test_dir,
    ...
    def test_PC_single_rosenblatt(self):
        ...
        self.assertEqual(
            self.uncertainty.data["a"].arguments["uncertain_parameters"], "a"
        )
        self.assertEqual(
            self.uncertainty.data["a"].arguments["method"], "collocation"
        )
        ...
        self.assertEqual(
            self.uncertainty.data["b"].arguments["uncertain_parameters"], "b"
        )
        self.assertEqual(
            self.uncertainty.data["b"].arguments["method"], "collocation"
        )
    ...
    def test_quantify_monte_carlo(self):
        ...
        self.assertEqual(self.uncertainty.data.arguments["seed"], self.seed)
        self.assertEqual(
            self.uncertainty.data.arguments["nr_samples"], self.nr_mc_samples
        )
    ...
    def test_custom_uncertainty_quantification(self):
        ...
        data = self.uncertainty.custom_uncertainty_quantification(
            custom_keyword="value", data_folder=self.output_test_dir, plot=None
        )
    ...
    def test_custom_uncertainty_quantification_arguments(self):
        ...
        self.uncertainty.uncertainty_calculations.custom_uncertainty_quantification = (
            custom
        )
