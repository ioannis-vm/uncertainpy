...
def generate_data_polynomial_chaos():  # pragma: no cover
    ...
    test = un.UncertaintyQuantification(
        model,
        features=features,
...
def generate_data_polynomial_chaos_single():  # pragma: no cover
    ...
    test.polynomial_chaos_single(data_folder=test_data_dir, seed=seed, plot=None)
...
def setup_mock_data():  # pragma: no cover
    ...
    for data_type in data_types:
        data["feature1d"][data_type] = np.array([1.0, 2.0])
        data["TestingModel1d"][data_type] = np.array([3.0, 4.0])
...
def generate_data_data_irregular():  # pragma: no cover
    ...
    for data_type in data_types:
        data["feature1d"][data_type] = [1.0, 2.0]
        data["TestingModel1d"][data_type] = [3.0, 4.0]
...
def generate_data_uncertainty_calculations():  # pragma: no cover
    ...
    uncertainty_calculations = un.core.UncertaintyCalculations(
        model=model, features=features, parameters=parameters
    )

    data = uncertainty_calculations.polynomial_chaos(
        uncertain_parameters="a", seed=seed
    )
    ...
    data = uncertainty_calculations.polynomial_chaos(
        uncertain_parameters="b", seed=seed
    )
