...
class Parallel(Base):
        ...
        for feature in result:
            if feature in self.features.interpolate or (
                ...
                if not is_regular(result[feature]["values"]):
                    raise ValueError(
                        "{}: values within one evaluation is irregular,".format(
                            feature
                        )
                ...
                if not is_regular(result[feature]["time"]):
                    raise ValueError(
                        "{}: times within one evaluation is irregular,".format(
                            feature
                        )
                        + " unable to perform interpolation."
                    )
                ...
                if np.ndim(result[feature]["values"]) == 0:
                    ...
                    result[feature]["interpolation"] = self.interpolation_1d(
                        result, feature
                    )
    ...
    def interpolation_1d(self, result, feature):
        ...
        if np.ndim(result[feature]["values"]) != 1:
            raise ValueError(
                "Cannot create 1D interpolation as the values of {} are not 1D".format(
                    feature
                )
            )
        ...
        if np.ndim(result[feature]["time"]) != 1:
            raise ValueError(
                "Cannot create 1D interpolation as the time of {} is not 1D".format(
                    feature
                )
            )

        if contains_nan(result[feature]["values"]):
            msg = "{}: values contains np.nan or None values, unable to create 1D interpolation.".format(
                feature
            )
            logger.warning(msg)

        elif contains_nan(result[feature]["time"]):
            msg = "{}: time contains np.nan or None values, unable to create 1D interpolation.".format(
                feature
            )
            logger.warning(msg)
            ...
            try:
                interpolation = scpi.InterpolatedUnivariateSpline(
                    result[feature]["time"], result[feature]["values"], k=3
                )
            except Exception as error:
                msg = "{}: unable to interpolate using scipy.interpolate.InterpolatedUnivariateSpline(time, values, k=3)".format(
                    feature
                )
                if not error.args:
                    error.args = ("",)
    ...
    def run(self, model_parameters):
        ...
        try:
            ...
            print("")
            print(
                "Caught exception when running/postprocessing model: {} in parallel:".format(
                    self.model.name
                )
            )
            print(
        ...
        try:
            ...
            for feature in feature_results:
                ...
                results[feature] = {"values": values_feature, "time": time_feature}
            ...
            print("")
            print(
                "Caught exception when calculating/postprocessing features of model: {} in parallel:".format(
                    self.model.name
                )
            )
            print(
